...
var PRESET_ANALYTICS = {
  portrait: {
    ...
    aiRecommendationMarkdown: `### \u{1F4F8} Professional Portrait Assessment
...
* **Suggested Action**: Use the **Acne & Skin Smooth** utility at 45% power to preserve raw skin pores while blending micro-distractions.`
  ...
  product: {
    ...
    aiRecommendationMarkdown: `### \u{1F9EA} Premium Commercial Product Diagnostics
...
* **Ad-Concept Option**: Convert this directly into a high-converting Instagram story mockup using **Smart Resize**.`
  ...
  old_photo: {
    ...
    aiRecommendationMarkdown: `### \u{1F570}\uFE0F Archival Restoration Plan
...
function getOfflineAnalyzeFallback(presetName, userDescription) {
  const baseData = PRESET_ANALYTICS[presetName] || PRESET_ANALYTICS.portrait;
...
function getOfflineAssistantResponse(message, activeImageDetails) {
  const msg = message.toLowerCase();
  let responseText = `### \u{1F4A1} AI Media Studio Assistant Response (Local Mode)
...
*Caption Recommendation*: \u{1F680} Product storytelling redefined with AI Media Studio Ultra. #graphicdesign #photography #foryou #visualart`;
...
app.post("/api/analyze", async (req, res) => {
  ...
  try {
    ...
    const timeoutPromise = new Promise(
      (_, reject) => setTimeout(() => reject(new Error("Gemini API call timed out after 3.5s")), 3500)
    );
